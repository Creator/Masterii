 ...
 */
public class Fragment_SearchedAnime extends Fragment {
    ...
    public Fragment_SearchedAnime(Context context, Menu menuInstance, String seriesType) {
    ...
    Callback seriesGet_Callback = new Callback() { // Required in order to get the 'Remember me' cookie.
        ...
        public void onResponse(Response response) {
            ...
            try {
                ...
                e.printStackTrace();
                ((Activity_Search)context).runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        dialog.hide();
                    }
    ...
    public void seriesAttempt(String responseBody) {
        ...
        try {
            if (((Activity_Search)context).jsonArray_anime == null)
                ((Activity_Search) context).jsonArray_anime = new JSONArray(responseBody);
            JSONArray tmpArray = new JSONArray();
            for (int i = 0;i<((Activity_Search)context).jsonArray_anime.length();i++) {
                if (((Activity_Search)context).jsonArray_anime.getJSONObject(i).getString("type").equals(seriesType))
                    tmpArray.put(((Activity_Search)context).jsonArray_anime.getJSONObject(i));
            }
    ...
    public void setUserVisibleHint(boolean isVisibleToUser) {
        ...
        if (!ranOnce) {
            ...
            if (((Activity_Search) context).jsonArray_anime == null) {
                ...
                try {
                    Request seriesGet_Req = new Request.Builder()
                            .url("http://www.masterani.me/api/anime-search?keyword=" + URLEncoder.encode(((Activity_Search) context).keyword_search, "UTF-8"))
                            .header("User-Agent", getString(R.string.app_name) + " Android App")
                            .addHeader("Accept", "application/json, text/javascript, */*; q=0.01")
                            .build();
            ...
            } else if (((Activity_Search) context).jsonArray_anime != null) {
                new Thread(new Runnable() {
                    @Override
                    public void run() {
                        seriesAttempt(null);
